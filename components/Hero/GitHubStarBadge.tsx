...
interface GitHubStarBadgeProps {
  repoFullName: string; // e.g. 'NiladriHazra/WhatToBuild'
  className?: string;
  compact?: boolean;
}
...
export default function GitHubStarBadge({ repoFullName, className, compact = false }: GitHubStarBadgeProps) {
  ...
  // Fetch repository data from our API route (avoids rate-limit issues on client)
  useEffect(() => {
    let cancelled = false;
    const fetchStars = async () => {
      try {
        const res = await fetch(`/api/github-repo/${encodeURIComponent(repoFullName)}`);
        if (!res.ok) return;
