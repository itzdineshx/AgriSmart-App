...
export default function SignUp() {
  ...
  const handleEmailSignUp = async (e: React.FormEvent) => {
    ...
    try {
      // This will throw an error if Firebase is not configured
      const userCredential = await createUserWithEmailAndPassword(auth, email, password);

      // Note: Display name setting is disabled for now
      ...
      <Card className="w-full max-w-md">
        <CardHeader className="space-y-1">
          ...
          <CardDescription className="text-center">
            Join AgriSmart to access farming insights
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          {!isFirebaseConfigured && (
            <div className="text-center text-sm text-muted-foreground">
              ...
              <p className="text-xs mt-1">Only email/password registration available</p>
            </div>
